...
def resolve_default_sample(config):
    ...
    try:
        ...
        gdown.download(
            id=config.StreamConfig.gdrive_mix_id,
...
def main(args, config):
    ...
    converter_outputs = list(filter(
        lambda x: re.match(r".*_outer_stft_.*\.tflite$", x),
        converter_outputs
    ))
    converter_outputs.sort(key=lambda x: os.stat(x).st_mtime, reverse=True)

    tflite_model_path = str(
        config.ConverterConfig.tflite_model_dst + 
        f"\{converter_outputs[0]}"
    )
    ...
    stream_class = TFLiteTorchStream(
        tflite_model_path,
        segment=float(parsed_segment),
    )
    out_paths = stream_class(
        track_path,
        args.out_dir,
        args.duration
    )
